...
class Gmap {
	...
	/**
	 *
	 * Marker to select a position on the map.
	 * This var is the position inside the markers array where this marker is.
	 * @var int
	 */
	...
	var $marker_clusterer_options = array(
	 ...
	 var $driving_dir_text = array(
             ...
             */
            function GoogleMapAPI($map_id = 'map', $app_id = '') {
            	$this->map_id = $map_id;
            	$this->sidebar_id = 'sidebar_' . $map_id;
            	$this->app_id = $app_id;
            }

            function addDraggableMarker($address=null){
            	if($address != null)
            	$marker_pos = $this->drag_marker = $this->addMarkerByAddress($address,'','','','','', true);
            	else
            	$marker_pos = $this->drag_marker = $this->addMarkerByCoords(0, 0,'','','','','', true);
            	return $marker_pos;
            }

            function getDragMarkerPosition(){
            	$array_marker = $this->_markers[$this->drag_marker];
            	return array($array_marker['lon'], $array_marker['lat']);
            }
             ...
             * @return int|bool
             */
            function addMarkerByAddress($address,$title = '',$html = '',$tooltip = '', $icon_filename = '', $icon_shadow_filename='', $draggable=false) {
            	if(($_geocode = $this->getGeocode($address)) === false)
            	return false;
            	return $this->addMarkerByCoords($_geocode['lon'],$_geocode['lat'],$title,$html,$tooltip, $icon_filename, $icon_shadow_filename, $draggable);
            }
             ...
             * @return int|bool
             */
            function addMarkerByCoords($lon,$lat,$title = '',$html = '',$tooltip = '', $icon_filename = '', $icon_shadow_filename='', $draggable=false) {
            	$_marker['lon'] = $lon;
            	$_marker['lat'] = $lat;
            ...
            function getAddMarkersJS($map_id = "", $pano= false) {
            	...
            	foreach($this->_markers as $_marker) {
            		...
            		$_output .= "var draggable = true;";
            		else
            		$_output .= "var draggable = false;";
            		$_output .= sprintf('%s.push(createMarker(%s%s, point,"%s","%s", %s, %s, "%s", %s, draggable ));',
            ...
            function getCreateMarkerJS() {
            	$_output = "
    	   var markers = Array(); var drag_marker = null; var max_distance = 0.02; //degrees
    	   function createMarker(map, point, title, html, icon, icon_shadow, sidebar_id, openers, draggable){
    	   		var marker_options = {
			        position: point,
			        map: map,
			        title: title,
			    ...
			    if(html!=''){
			        ...
			        google.maps.event.addListener(new_marker, '".$this->window_trigger."', function() {
			          infowindow.open(map,new_marker); /*alert('hola');*/
			        });
                	...
                	google.maps.event.addListener(new_marker, 'dragend', function() {
                		for(var i=0; i<markers.length; i++){
                			var distancex = drag_marker.position.lng() - markers[i].position.lng();
                			var distancey = drag_marker.position.lat() - markers[i].position.lat();
		...
		";
            ...
            function getStaticImage($lat, $lon, $width, $height) {
            	$url = "http://maps.google.com/staticmap?center=$lat,$lon&zoom=15&size={$width}x{$height}&maptype=mobile&markers=$lat,$lon,red&key=&sensor=false";
            	return $url;
            }
